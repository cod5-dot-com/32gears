...
/*************** Structures ***************/
#ifdef LINUX
   void Clongjmp(jmp_buf env, int v);
   int Csetjmp(jmp_buf env);
   #undef setjmp 
   #define longjmp Clongjmp
   #define setjmp Csetjmp
   typedef struct jmp_buf2 {  
      uint32 sp, fp, pc, extra[10];
   } jmp_buf2;
#elif defined(WIN32)
   #ifndef setjmp
   ...
   for(;;)
   {
      ...
      if(SimulateIsr && (int)arg == OS_CPU_COUNT-1)
      {
         ...
         for(;;)
         {
            ...
            if(value == 0)
               break;
            OS_InterruptServiceRoutine(value, (void*)0);
            value = 0;
         }
      }
#endif
